...
\newpage
\appendix
\section{Hist\'orico de Revis\~oes}
\label{sec:revisoes}

Este ap\^endice registra as altera\c{c}\~oes feitas no GDD ap\'os a entrega original da P1, garantindo rastreabilidade entre vers\~oes do documento.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l l X}
    \toprule
    \textbf{Vers\~ao} & \textbf{Data} & \textbf{Altera\c{c}\~oes} \\
    \midrule
    1.0 & 02/mar/2026 & Vers\~ao inicial entregue na P1. \\
    \addlinespace
    1.1 & 09/mai/2026 & Revis\~ao de coer\^encia interna pr\'e-entrega da P2:
      \begin{itemize}\setlength{\itemsep}{0pt}
        \item Se\c{c}\~ao 2.3.4 (Gera\c{c}\~ao): faixa de salas dos Andares 1 e 2
          ajustada de ``entre 5 e 8 salas cada'' para ``5 a 6 (Andar 1) e 6 a 7
          (Andar 2)'', alinhando com os valores espec\'ificos da Se\c{c}\~ao 5.2.
        \item Se\c{c}\~ao 2.3.4 (Gera\c{c}\~ao): adicionada refer\^encia cruzada
          \texttt{\textbackslash ref\{sec:progressao\}} para a se\c{c}\~ao de
          progress\~ao detalhada.
        \item Se\c{c}\~ao 2.3.2 (Vida): redacao sobre escassez de cura
          reescrita para clarear que a raridade \'e \emph{progressiva por andar},
          eliminando a tens\~ao com a descri\c{c}\~ao de ``drops generosos no
          Andar 1'' presente na Se\c{c}\~ao 5.2.
        \item Adi\c{c}\~ao deste ap\^endice de hist\'orico de revis\~oes.
      \end{itemize} \\
    \bottomrule
  \end{tabularx}
\end{table}

As altera\c{c}\~oes acima n\~ao mudam o escopo ou o conceito do jogo. Trata-se exclusivamente de \emph{polimento textual} para eliminar pontos de fric\c{c}\~ao detectados em revis\~ao sistem\'atica. Toda diverg\^encia entre o que o GDD prescreve e o que o jogo entrega est\'a documentada no Cap\'itulo 8 da \emph{Documenta\c{c}\~ao T\'ecnica} (\texttt{docs/main.pdf}), na se\c{c}\~ao ``Mudan\c{c}as de escopo em rela\c{c}\~ao ao GDD''.
