...
SoundGenerator.prototype._genSound = function(n, chnBuf, currentpos) {
	...
	for (var j = numSamples; j >= 0; --j) {
		...
		else if (j >= this.attack + this.sustain) e -= (j - this.attack - this.sustain) / this.release;
		...
		if (this.instr.noise_fader) rsample += (2 * _math.random() - 1) * this.instr.noise_fader * e;
		rsample *= e / 255;
		...
		switch (this.instr.fx_filter) {
			...
			case 2: rsample = low; break;
			case 3: rsample = band; break;
			case 4: rsample = low + high; break;
		}
...
MusicGenerator.prototype._generateTrack = function(instr, mixBuf, callBack) {
	...
	var currentpos = 0, p = 0, row = 0;
	var recordSounds = function() {
		var beginning = Date.now();
		while (true) {
			if (row === 32) { row = 0; p += 1; continue; }
			if (p === endPattern - 1) return finalize();
			var cp = instr.p[p];
			if (cp) {
				var n = instr.c[cp - 1].n[row];
				if (n) soundGen._genSound(n, chnBuf, currentpos);
			}
			currentpos += rowLen;
			row += 1;
	...
	var finalize = function() {
		...
		for (var b2 = 0; b2 < waveSamples; b2++) mixBuf.left[b2] += chnBuf.left[b2];
		for (var b2 = 0; b2 < waveSamples; b2++) mixBuf.right[b2] += chnBuf.right[b2];
		callBack();
	};
	recordSounds();
};
